...
const Menu = ({ buttons, value, setValue }: MenuProps) => {
  ...
  return buttons.map((button: any, index: number) => (
    <div className='mb-2' key={index}>
      <button
        className={`flex items-center w-full px-3 py-1 rounded-full border-2 border-transparent font-semibold transition-colors bg-n-2_hover tap-highlight-color text-n-4 ${
